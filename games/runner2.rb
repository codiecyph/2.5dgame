...
class RunningHeroWindow < Gosu::Window
    ...
    @background = Gosu::Image.new("assets/background3.png")
...
#	@enemy = Gosu::Image::new("assets/enemy.png")
	...
	#creating enemy
	@enemy_images = Gosu::Image::load_tiles("assets/cub.png", 64, 64)
	@enemies = [Enemy.new(self, @enemy_images, 200, 331, :left)]
	
	# npc
	@enemies1 = [Enemy.new(self, @enemy_images, 100, 331, :right)]
    @score = 0
	...
	@ambience = Gosu::Sample.new("assets/forest.wav")
	@ambience.play
	
	@jump_sound = Gosu::Sample.new("assets/jmp.wav")
	@walk_sound = Gosu::Sample.new("assets/grass.wav")
	@squash_sound = Gosu::Sample.new("assets/squash.wav")
	@argh_sound = Gosu::Sample.new("assets/argh.wav")
...
def sound
    @@sound ||= Gosu::Song.new(
        $window, Game.media_path('assets/grass.wav')) 
end
	...
	def update
		...
		jump && @jump_sound.play if Gosu::button_down?(Gosu::KbUp)
		
		handle_jump if @jumping
		
		@enemies.each{|enemy| enemy.update}
		# npc
		@enemies1.each{|enemy| enemy.update}
	...
	def draw
		...
		# npc
		@enemies1.each{|enemy| enemy.draw}
		draw_collision_bodies
...
private	
	...
	def move(way)
		...
		@walking = true
		if way == :right
		  @hero_position = [@hero_position[0] + speed, @hero_position[1]]
		  @hero_direction = :right
		else
		  @hero_position = [@hero_position[0] - speed, @hero_position[1]]
		  @hero_direction = :left
